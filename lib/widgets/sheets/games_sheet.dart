...
class _GamesSheetState extends State<GamesSheet> {
  final matchesList = <GameMatch>[];

  @override
  void initState() {
    super.initState();
    for (int i = 0; i < widget.matchesNumber; i++) {
      matchesList.add(GameMatch.empty());
    }
  }

  @override
  Widget build(BuildContext context) {
    final canSave = matchesList.every((match) => match.canSave);
    return Material(
            ...
            children: [
                      ...
                      child: MatchCard(
                        ...
                        chooseFirstTeam: () {
                          showListTileDialog(
                            ...
                            (name) {
                              if (match.secondTeam == name) {
                                showOverlayMessage(
                                  "The first team can't be the same as the first team",
                                );
                                return;
                              }
                              setState(() => match.firstTeam = name);
                            },
                          );
                        },
                        chooseSecondTeam: () {
                          showListTileDialog(
                            ...
                            (name) {
                              ...
                              setState(() => match.secondTeam = name);
                            },
                          );
                        },
                        chooseDate: () {
                          showCalendar(
                            context,
                            match.created,
                            (date) => setState(() => match.created = date),
                          );
                        },
                        chooseLocation: () {
                          showListTileDialog(
                            ...
                            (name) => setState(() => match.location = name),
                          );
                        },
                      ),
                    );
              ...
              CustomButton1(
                ...
                onTap: () {
                  ...
                  widget.onSave.call(matchesList);
